...
const SmokeyCursor = ({ onTextOver = () => {} }) => {
  ...
  // Function to check if cursor is over text elements
  const checkTextElements = (x, y) => {
    const textElements = document.querySelectorAll('.smoke-text, h1, h2, h3, p, span');
    let overText = false;
  ...
  useEffect(() => {
    ...
    const initFluid = () => {
      ...
      const updatePointerMoveData = (pointer, posX, posY, color) => {
        ...
        // Update cursor position for text detection
        const rect = canvas.getBoundingClientRect();
        const cursorX = posX / window.devicePixelRatio + rect.left;
        const cursorY = posY / window.devicePixelRatio + rect.top;
        setCursorPosition({ x: cursorX, y: cursorY });
        checkTextElements(cursorX, cursorY);
      };
