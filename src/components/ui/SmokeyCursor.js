...
const SmokeyCursor = (props) => {
  ...
  const CONSTANT_COLOR = {
    r: parseInt(HEX_COLOR.substr(1, 2), 16) / 255,
    g: parseInt(HEX_COLOR.substr(3, 2), 16) / 255,
    b: parseInt(HEX_COLOR.substr(5, 2), 16) / 255
  };
  ...
  useEffect(() => {
    ...
    const initFluid = () => {
      ...
      function pointerPrototype() {
        ...
        this.down = false;
        this.moved = false;
        // Use the constant orange color directly
        this.color = { 
          r: CONSTANT_COLOR.r, 
          g: CONSTANT_COLOR.g, 
          b: CONSTANT_COLOR.b 
      ...
      // Start with some random splats using the constant color
      for (let i = 0; i < 5; i++) {
        const x = Math.random();
        const y = Math.random();
