...
export function MarqueeRL({
    ...
    const updateDimensions = () => {
      if (contentRef.current && contentRef.current.children.length > 0) {
        ...
        if (firstItem) {
          ...
          // Calculate total set width
          const itemCount = React.Children.count(children);
          const totalWidth = (width * itemCount) + (gapNum * (itemCount - 1));
          setTotalSetWidth(totalWidth);
          
          // Initialize animation position for reverse
          animX.current = -totalWidth;
        }
      }
  ...
  useAnimationFrame((t, delta) => {
    if (!containerRef.current || !contentRef.current || itemWidth === 0 || !isInitialized || totalSetWidth === 0) return;
    ...
    // Reset when we've moved one full set to the right (reached 0)
    if (animX.current >= 0) {
