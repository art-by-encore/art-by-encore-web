...
const BreadCrumbs = () => {
    ...
    async function getPortfolioData() {
        try {
            ...
            console.log('connection check:', { healthCheck, healthError });
        ...
        <ul className='flex text-white flex-row flex-wrap font-footer-text gap-[20px] mb-[30px]'>
            {
                displayData?.map((item, index) => {
                    ...
                    const title = item.title
                        ...
                        || item.seo?.title
                        || 'Portfolio';
                    const finalURL = item?.content?.card?.isExternalURL ? item.content.card.pageUrl : href;
                    const target = item?.content?.card?.isExternalURL ? '_blank' : '_self';
                    return (
                        <li className='text-orange underline' key={item.id || index}>
                            <Link href={finalURL} target={target} className=''>
                                <RollText text={title} />
                            </Link>
                        </li>
                    )
